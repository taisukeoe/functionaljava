...
public abstract class List<A> implements Iterable<A> {
  ...
  public final Stream<A> toStream() {
    return foldRight(a -> as -> as.cons(a), Stream.nil());
  }
  ...
  public final List<List<A>> partition(final int n) {
    ...
    return unfold(
      as ->as.isEmpty() ? Option.none() : some(as.splitAt(n))
    , this);
  }
  ...
  public final List<A> nub(final Equal<A> eq) {
    return isEmpty() ? this : cons(head(), tail().filter(
      a -> !eq.eq(a, head())
    ).nub(eq));
  }
  ...
  public final <B> Option<List<B>> mapMOption(final F<A, Option<B>> f) {
    return foldRight(
      (a, bs) -> f.f(a).bind(b -> bs.map(bbs -> bbs.cons(b)))
    ...
    return unfold(
        new F<A, Option<P2<A, A>>>() {
          public Option<P2<A, A>> f(final A o) {
            return Option.iif(p2 -> p.f(o), P.p(o, f.f(o)));
          }
        }
        , a);
  }
