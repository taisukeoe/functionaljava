...
public abstract class Either<A, B> {
  ...
  public static <A, X> Either<List<A>, X> sequenceLeft(final List<Either<A, X>> a) {
    return a.isEmpty() ?
           ...
           a.head().left().bind(aa ->
             sequenceLeft(a.tail()).left().map(cons_(aa))
           );
  }
  ...
  public static <B, X> Either<X, List<B>> sequenceRight(final List<Either<X, B>> a) {
    return a.isEmpty() ?
           ...
           a.head().right().bind(bb ->
             sequenceRight(a.tail()).right().map(cons_(bb))
           );
  }
  ...
  public static <A, B> List<A> lefts(final List<Either<A, B>> es) {
    return es.foldRight(e -> as ->
      e.isLeft() ? as.cons(e.left().value()) : as
    , List.nil());
  }
  ...
  public static <A, B> List<B> rights(final List<Either<A, B>> es) {
    return es.foldRight(e -> bs ->
      e.isRight() ? bs.cons(e.right().value()) : bs
    , List.nil());
  }
}
